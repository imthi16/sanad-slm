...
const button = cva(
    ...
    "duration-150 focus-visible:outline-2 disabled:opacity-50 disabled:pointer-events-none",
  {
    variants: {
      variant: {
        primary: "bg-brass-400 text-dune-950 hover:bg-brass-400/90",
        ghost: "text-sand-400 hover:text-sand-100 hover:bg-dune-700/40",
        outline: "border border-dune-700 text-sand-100 hover:border-brass-400/60",
        danger: "bg-claret-500 text-sand-100 hover:bg-claret-500/90",
      },
      size: {
...
const badge = cva(
  "inline-flex items-center gap-1.5 rounded-full border ps-2.5 pe-2.5 py-0.5 text-xs font-medium",
  {
    variants: {
      tone: {
        brass: "border-brass-400/40 text-brass-400",
        teal: "border-teal-400/40 text-teal-400",
        sand: "border-dune-700 text-sand-400",
        claret: "border-claret-500/50 text-claret-500",
      },
    },
...
export function Skeleton({ className }: { className?: string }) {
  return <div className={clsx("animate-pulse rounded-md bg-dune-700/50", className)} aria-hidden />;
}
