...
[![Python 3.12](https://img.shields.io/badge/python-3.12-1FA79B?style=flat-square)](./ml/pyproject.toml)
[![React 19](https://img.shields.io/badge/react-19-1FA79B?style=flat-square)](./apps/web/package.json)
[![just check](https://img.shields.io/badge/just%20check-green-2ea043?style=flat-square)](./justfile)
[![RTL first](https://img.shields.io/badge/عربي-RTL%20first-141C2B?style=flat-square)](#-arabic-is-a-first-class-citizen)
...
- [Evaluation: the credibility core](#-evaluation-the-credibility-core)
- [Arabic is a first-class citizen](#-arabic-is-a-first-class-citizen)
- [Sovereignty, verified](#-sovereignty-verified)
- [Results](#-results)
...
traces to a report file in [`ml/evals/reports/`](./ml/evals/reports/) by hash, and
judge-based claims are invalid without a human-validation κ attached.

📐 **[`CLAUDE.md`](./CLAUDE.md) is the single source of truth** — architecture, prime
directives, pins, gates, and the phased roadmap. When this README and CLAUDE.md disagree,
CLAUDE.md wins.
...
Sovereign AI in the Gulf is usually pitched top-down: giant models, giant clusters, giant
claims. Sanad tests the opposite corner of the design space, end to end:
...
```
                 ...
                 │  Terraform: gpu_train ──▶ uv env ──▶ Unsloth QLoRA+DoRA (Qwen3-4B)
                 │        │                                    │
                 │        ▼                                    ▼
...
One contract everywhere: the **OpenAI Chat Completions dialect**. The gateway's
...
**Fertility ≈ latency ≈ cost ≈ effective context.** An English-first tokenizer shatters
Arabic words into fragments — every fragment is a token you pay for, wait for, and burn
context on. Sanad measures **tokens/word** for five tokenizers (Qwen3, jais, ALLaM,
Falcon-H1, Llama-3.2) over three fixed corpora (MSA news, Arabic banking, English), exposes
it live at `POST /v1/tokenize/fertility`, and makes it *visible*:

> **The FertilityField hero** — the landing page renders a real bilingual banking sentence
> as ~1,200 instanced glyph particles (Arabic shaped in the DOM first, so ligatures stay
> correct). Switch tokenizers and watch the particles regroup into token clusters — teal for
> Arabic tokens, sand for English — with a live HUD: `tokens · tokens/word · Δcost vs best`.
> Watching an English-first tokenizer shatter Arabic words makes the argument better than
> any table.
...
One env var rules them all — `SANAD_MODE` — and everything (pydantic settings, compose
profiles, Helm values, the web build) derives from it:
...
```bash
git clone <this-repo> && cd sanad-slm
...
just check     # the full PR gate: lint + types + tests (all 3 stacks)
               #   + data-gate + verify-no-cdn        ← currently GREEN ✅
```

Then open **http://localhost:5173** — flip the language toggle and watch the whole app go RTL.
...
The overlay sets `SANAD_MODE=sovereign`, exports all offline HF flags, and places every
service on an `internal: true` Docker network — outbound routing is impossible at the
network layer, not just discouraged. The runbook's checklist verifies it.
</details>
...
`just --list` is the only entry point you need. The full surface:
...
| | `just fertility` | regenerate `fertility.json` for the API + 3D hero |
...
Two **separate** uv workspaces by design: `ml/` carries the CUDA tree, `apps/api` stays
slim enough to cold-start in seconds. Their dependency graphs never merge.
...
(800–1,500, CC-BY-4.0)   ─┘        │
              ...
              seed 3407 · NF4 · r16/α16 · <16 GB peak VRAM · <$50 · MLflow-logged
                                   │
                     ┌─────────────┴─────────────┐
                     ▼                           ▼
...
Records carry `provenance: native | translated | synthetic` — and that split is printed
into **every** eval report. Machine-translated eval sets presented as native are how Arabic
NLP numbers lie; Sanad's rigor signal is making the split unhideable.

---

## ⚖️ Evaluation: the credibility core

**Benchmarks** — pinned lm-evaluation-harness commit, identical command for every model in
the matrix (base Qwen3-4B, fine-tuned, ALLaM-7B, jais-6.7b, Falcon-H1-3B/7B): ArabicMMLU,
AraTrust, MadinahQA, ALRAGE. Vendor numbers are quoted only *next to* our re-measured ones.

**Domain eval** — `sanad_bank_eval_v1`: 300 own-authored held-out items
(150 AR / 120 EN / 30 code-switch) across extraction, classification, and grounded QA.
sha256-frozen, never enters training, treated as private (BALSAM-style contamination hygiene).

**3C3H multi-judge** — Correctness is a **binary gate** (fail → score 0); then
Completeness, Conciseness, Helpfulness, Honesty, Harmlessness, each 1–5, rubrics in both
languages. Judge-pool rule: **never a judge from the tested model's family**
(self-preference bias) — Qwen under test ⇒ Falcon-H1-7B + ALLaM-7B judge, locally via vLLM.

**Disagreement is data** — Krippendorff's α overall and per-dimension, pairwise Cohen's κ,
a judge×dimension disagreement heatmap on the dashboard, and any item with judge spread ≥ 2
...
- **RTL everywhere.** `dir` flips at `<html>`; components use logical CSS properties only
  (`ms-*`, `pe-*`, `text-start`) — a CI grep **fails the build** on `ml-|mr-|pl-|pr-|text-left|text-right`.
- **Streaming that never tears ligatures.** SSE deltas buffer and flush on grapheme
  boundaries (`Intl.Segmenter`), holding back the last grapheme while it can still grow
  (lam-alef, shadda chains). Unit-tested with real Arabic.
- **Dual-script typography as identity.** Space Grotesk + Inter for Latin; IBM Plex Sans
  Arabic + Amiri for Arabic — all self-hosted, subsetted, zero CDN.
- **Numerals done right.** `Intl.NumberFormat` per locale with a toggle for Eastern Arabic
  numerals — the UAE's `arab` system (٠١٢٣), *not* the Persian/Urdu `arabext` set (a
  distinction our own test suite caught).
- **Every i18n key exists in both catalogs** — `en/*.json` without its `ar/*.json` sibling
  fails CI; machine-drafted Arabic is flagged `pending-native` until reviewed.
- **Playwright snapshots run RTL *and* LTR for every page.** Bidi regressions are the #1
  bug class in bilingual UIs; here they're a red build.

---

## 🛡 Sovereignty, verified

Sovereignty is enforced at four independent layers — belt, braces, and two more belts:

1. **Process** — `SANAD_MODE=sovereign` exports `HF_HUB_OFFLINE=1`,
   `TRANSFORMERS_OFFLINE=1`, `HF_DATASETS_OFFLINE=1`; services **fail loudly** if a model
   dir is missing rather than silently fetching from a hub.
2. **Build** — `just verify-no-cdn` scans the built web bundle: zero external origins in
   fetchable HTML/CSS contexts, and a justified allowlist for inert JS strings. It has
   already caught a real leak (a 3D text library's CDN font fallback — now pinned to
   self-hosted fonts).
3. **Runtime** — nginx ships CSP `default-src 'self'`; the sovereign compose network is
   `internal: true`; K8s gets default-deny egress NetworkPolicies via the
   `sovereign-guard` chart.
4. **Observability** — the `SanadSovereignEgress` PrometheusRule: sustained non-cluster
   egress from the sovereign namespace fires a **critical alert**. The checklist requires
   it green for 24 h before any demo. *This alert firing = broken promise.*

Plus supply-chain hygiene: Trivy-scanned images (0 high/critical), Syft SBOMs,
cosign-signed images *and* model manifests, SOPS+age for every secret, and PII-scrubbing
structlog processors (emails, UAE IBAN, Emirates ID, phone — Arabic and Western digits)
with chat content **not persisted** by default.

---

## 📊 Results

> **Honest-claims policy** (prime directive 5): this table populates only from hashed
> report files in `ml/evals/reports/`. The pipeline is built and gated; training and
> evaluation runs land at P2–P4. No number appears here before its report exists.

| Metric | Base Qwen3-4B | Sanad (fine-tuned) | ALLaM-7B | jais-6.7b | Falcon-H1-7B* |
|---|---|---|---|---|---|
...
The release gate is honest by construction: fine-tuned must beat base by **≥ +5 pts**
in-domain while staying within **−1 pt** on ArabicMMLU (no catastrophic forgetting), or the
build is red.
...
| Layer | Tools | Gate |
|---|---|---|
| `ml/` | ruff · mypy --strict · pytest | 16/16 — schema validators, PII scan, license gate blocks a planted non-commercial record |
| `apps/api` | ruff · mypy --strict · pytest · schemathesis | 16/16 — SSE chunk-integrity, PII scrubbing (AR+EN digits), sovereign config forcing; coverage ≥ 80% in CI |
| `apps/web` | biome · tsc --strict · vitest · Playwright | 13/13 — grapheme buffering, atlas shaping safety, numeral systems; RTL+LTR snapshots per page |
| data | `just data-gate` | licenses ∈ {Apache-2.0, CC-BY-4.0, MIT} for commercial profile |
...
| **P7 · Position + publish** | README numbers, model card, workshop paper draft | ⏳ |

---

## 🤝 Contributing

The working agreements (CLAUDE.md §14), abridged:
...
4. Every `en` string ships with its `ar` sibling (CI-enforced); real MSA, with
   machine-drafted strings flagged for native review.
5. Generated artifacts (TS client, `fertility.json`, reports) are never hand-edited.
6. Every number in README/paper traces to a hashed report file.
...
[jais](https://huggingface.co/inceptionai) (Arabic-native comparators/judges) ·
...
Code: **Apache-2.0** ([LICENSE](./LICENSE)) · Own datasets & docs: **CC-BY-4.0**.
Model/data licensing guardrails are CI-enforced — see
